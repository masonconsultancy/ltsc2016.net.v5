...
# Modified to allow Tls1.2 when downloading the .net files using the switch -UseTls12
# Modified Prepend-Sdk-InstallRoot-To-Path method to allow path changes to persist out of session
#
...
.PARAMETER UseTls12
    Default: false
    Uses Tls1.2 to download the file
#>
[cmdletbinding()]
param(
   [string]$Channel="LTS",
...
function Prepend-Sdk-InstallRoot-To-Path([string]$InstallRoot, [string]$BinFolderRelativePath) {
    ...
    if (-Not $NoPath) {
        ...
        if (-Not $env:path.Contains($SuffixedBinPath)) {
            ...
            $newPath = $SuffixedBinPath + $env:path
            Say "New PATH: `"$newPath`"."
